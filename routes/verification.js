		...
		(err, token) => {
			if (err) {
				...
				mailer(
					...
					`Please follow this link to verify your account: http://localhost:2000/verification/confirm/${token}
...
router.get("/confirm/:id", async (req, res) => {
	...
	await UserModel.findById(decoded.user_id, (error, result) => {
		if (error) {
			...
			<a href='http://localhost:3000/login'>Back to log in</a>`);
			console.log(error);
		} else {
			...
			result.save((error, result) => {
				...
				<a href='http://localhost:3000/login'>Back to your account</a>`);
			});
		}
	});
	//
